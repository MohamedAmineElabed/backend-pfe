...
public UserEntity registerUserFromDemande(String email, RegisterFromDemandeRequest request){
    ...
    if (userRepository.findByEmail(email).isPresent()){
        throw new ResponseStatusException(
            HttpStatus.CONFLICT, "Cet email utilisateur est déjà utilisé");
    }
    if (organismeRepository.existsByEmailOrganisme(demande.getEmailOrganisme())) {
        throw new ResponseStatusException(
            HttpStatus.CONFLICT, "Cet email d'organisme est déjà utilisé");
    }
...
public OrganismeResponse createOrganisme(OrganismeRequest request) {
   ...
   /*  public void assignResponsableToOrganisme(Long organismeId, ProfileRequest request) {
        OrganismeEntity organisme = organismeRepository.findById(organismeId)
            .orElseThrow(() -> new RuntimeException("Organisme not found with id: " + organismeId));
        UserEntity responsable= convertToUserEntity(request);
        responsable.setOrganisme(organisme);
        responsable.setEtat("actif");
        userRepository.save(responsable);
        organisme.setResponsable(responsable);
