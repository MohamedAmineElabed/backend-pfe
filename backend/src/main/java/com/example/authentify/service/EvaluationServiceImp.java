...
public class EvaluationServiceImp {
    ...
    public ReponseEntity saveReponse(Long evaluationId, Long critereId, Integer valeur, MultipartFile[] files) throws IOException {
        ...
        // 3. Recompute evaluation score / label / statut
...
return reponse;
    ...
    public void deleteFiles(List<PreuveEntity> preuves) {
        for (PreuveEntity preuve : preuves) {
            try {
                Path path = Paths.get(preuve.getCheminFichier());
                if (Files.exists(path)) {
                    Files.delete(path);
                }
            } catch (IOException ignored) {
                System.err.println("Failed to delete file: " + preuve.getCheminFichier());
            }
        }
    }

    // ─────────────────────────────────────────────────────────────
    // UPDATE EVALUATION STATUT (called internally after each response update, or manually from a specific endpoint)
    // ─────────────────────────────────────────────────────────────
