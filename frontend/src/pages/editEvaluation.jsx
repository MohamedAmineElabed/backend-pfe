...
export default function EvalEdit() {
  ...
  function isTreated(r) {
    return r?.statut === "validé" || r?.statut === "refusé";
}
...
// Fetch principes
  ...
  return principes.filter(principe => {
    ...
    const treatedPrincipes = useMemo(() => {
    if (!evaluation?.reponses || !principes.length) return [];
        const answeredCritereIds = new Set(evaluation.reponses.map(r => r.critereId));
        //const answeredCritereIds = new Set(evaluation.reponses.map(r => Number(r.critereId)));

    return principes.filter(principe =>
        (principe.pratiques || []).some(pratique =>
            (pratique.criteres || []).some(critere =>
                answeredCritereIds.has(critere.id)  // ← any response, regardless of statut
            )
        )
    );
}, [principes, evaluation]);
